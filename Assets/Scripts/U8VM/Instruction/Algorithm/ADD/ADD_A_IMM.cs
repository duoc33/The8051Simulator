...
using UnityEngine;
    ...
    public class ADD_A_IMM : InstructInfo
    {
        public ADD_A_IMM()
        {
            bytes = 2;
        ...
        public override void exec(_instruct instr)
        {
            ...
            //OV标志位处理
            //让两个8位数相加的结果与0x100比较，如果大于等于则进位了
            int bit_7 = result >= 0x100 ? 1 : 0;
            //保留两个数的高三位和第四位 0111 1111 ，看她们相加是否进一   
            int bit_6 = (acc & 0x7f) + (src & 0x7f) >= 0x80 ? 1 : 0;
            //OV只有在bit_7和 bit_6 其中一个为1的情况下，才会进一，所以将他们异或，相同为0，相异为1
